...
def clean_league_managers(db, league_id: str):
    delete_query = f"""DELETE FROM dynastr.managers where league_id = '{
        league_id}' """
    cursor = db.cursor()
...
def clean_league_rosters(db, session_id: str, user_id: str, league_id: str):
    delete_query = f"""DELETE FROM dynastr.league_players where session_id = '{
        session_id}' and league_id = '{league_id}'"""
    cursor = db.cursor()
    cursor.execute(delete_query)
    db.commit()
    cursor.close()
...
def clean_league_picks(db, league_id: str, session_id: str) -> None:
    delete_query = f"""DELETE FROM dynastr.draft_picks where league_id = '{
        league_id}' and session_id = '{session_id}'"""
    cursor = db.cursor()
    cursor.execute(delete_query)
    cursor.close()
    return
...
def clean_draft_positions(db, league_id: str):
    delete_query = (
        f"""DELETE FROM dynastr.draft_positions where league_id = '{
            league_id}'"""
    )
    cursor = db.cursor()
    cursor.execute(delete_query)
...
def insert_current_leagues(db, user_data: UserDataModel):
    ...
    delete_user_leagues_query = f"""DELETE FROM dynastr.current_leagues where user_id = '{
        user_id}' and session_id ='{session_id}'"""
    cursor = db.cursor()
...
def clean_player_trades(db, league_id: str) -> None:
    delete_query = (
        f"""DELETE FROM dynastr.player_trades where league_id = '{
            league_id}'"""
    )
    cursor = db.cursor()
    cursor.execute(delete_query)
...
def clean_draft_trades(db, league_id: str) -> None:
    delete_query = (
        f"""DELETE FROM dynastr.draft_pick_trades where league_id = '{
            league_id}'"""
    )
    cursor = db.cursor()
    cursor.execute(delete_query)
...
def get_trades(league_id: str, nfl_state: dict, year_entered: str) -> list:
    ...
    if nfl_state["season_type"] != "off":
        ...
        for week in range(1, leg + 1):
            transactions = make_api_call(f"https://api.sleeper.app/v1/league/{league_id}/transactions/{week}")
        ...
        transactions = make_api_call(f"https://api.sleeper.app/v1/league/{league_id}/transactions/1")
        all_trades.extend(transactions)
        trades_payload = [p for p in [
            i for i in all_trades] if p["type"] == "trade"]
